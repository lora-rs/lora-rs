...
//!   [`DevNonce`], [`JoinNonce`], [`NetId`], [`Frequency`]) are plain
//!   owned `[u8; N]` newtypes (`Copy`, `Eq`, `Hash`) with explicit
//!   wire-order/value conversions: the wire transmits them little-endian,
//!   and `from_value`/`value` convert accordingly.
...
//!   bytes live. All parse entry points reject frames with a non-R1 major
//!   version.
//! * Decryption happens in place: the caller passes the `&mut [u8]`
//!   explicitly, and the encrypted/decrypted distinction lives in the types.
//! * MAC command parsing ([`crate::maccommands`], [`crate::certification`],
//!   [`crate::multicast`]) yields `Result` per command and fuses after the
//!   first error, with the framing generated by the `CommandHandler` derive
//!   from the enums' `#[cmd]` attributes.
