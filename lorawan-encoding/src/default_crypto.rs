...
impl<T: AsRef<[u8]> + AsMut<[u8]>> DecryptedJoinAcceptPayload<T, DefaultFactory> {
    ...
    /// ```
    /// let mut data = vec![
    ///     0x20u8, 0x49u8, 0x3eu8, 0xebu8, 0x51u8, 0xfbu8, 0xa2u8, 0x11u8, 0x6fu8, 0x81u8, 0x0eu8,
    ///     0xdbu8, 0x37u8, 0x42u8, 0x97u8, 0x51u8, 0x42u8,
    /// ];
...
impl<T: AsRef<[u8]> + AsMut<[u8]>> DecryptedDataPayload<T> {
    ...
    /// let dec_phy =
    ///     lorawan::parser::DecryptedDataPayload::new(data, &nwk_skey, Some(&app_skey), 1).unwrap();
    /// ```
    pub fn new<'a>(
        data: T,
        nwk_skey: &'a AES128,
