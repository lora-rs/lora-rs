...
///
/// Distinct from [`Error`], which covers individual payload construction:
/// this one reports framing problems (the stream cannot be split into
/// commands at all).
...
impl core::fmt::Display for ParseError {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        match self {
            ParseError::UnknownCid(cid) => write!(f, "unknown MAC command CID 0x{cid:02x}"),
            ParseError::Truncated { cid } => {
...
pub trait MacCommandSet<'a>: Sized {
    ...
    fn parse_one(data: &'a [u8]) -> Result<(Self, usize), ParseError>;
}
