...
impl JoinAcceptCreator {
    ...
    pub fn set_app_nonce(&mut self, app_nonce: &AppNonce) {
        self.data[1] = app_nonce.0[0];
        self.data[2] = app_nonce.0[1];
        self.data[3] = app_nonce.0[2];
    }

    pub fn set_net_id(&mut self, net_id: &NwkAddr) {
        self.data[4] = net_id.0[2];
        self.data[5] = net_id.0[1];
        self.data[6] = net_id.0[0];
    }

    pub fn set_dev_addr(&mut self, dev_addr: &DevAddr) {
        self.data[7] = dev_addr.0[3];
        self.data[8] = dev_addr.0[2];
        self.data[9] = dev_addr.0[1];
        self.data[10] = dev_addr.0[0];
    }

    pub fn set_dl_settings(&mut self, dl_settings: DLSettings) {
    ...
    pub fn set_c_f_list(&mut self, ch_list: Vec<Frequency>) -> Result<bool, &str> {
        if ch_list.len() > 5 {
    ...
    pub fn build(&mut self, key: AES128) -> Result<&[u8], &str> {
        if !self.encrypted {
            let aes_enc = aessafe::AesSafe128Encryptor::new(&key.0[..]);
