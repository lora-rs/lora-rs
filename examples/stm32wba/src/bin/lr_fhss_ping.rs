...
async fn main(_spawner: Spawner) {
    ...
    lora_radio.set_irq_params(Some(RadioMode::Transmit))
        .await
        .unwrap();
    ...
    lora_radio.set_tx_power_and_ramp_time(TX_OUTPUT_POWER_DBM, None, true)
        .await
        .unwrap();
    ...
    info!("Starting LR-FHSS transmission every {} seconds...", INTER_PKT_DELAY_MS / 1000);
    ...
    loop {
        ...
        lora_radio.set_channel(RF_FREQUENCY).await.unwrap();
        lora_radio.set_tx_power_and_ramp_time(TX_OUTPUT_POWER_DBM, None, true).await.unwrap();
        ...
        info!("Using hop_sequence_id = {} (out of {} sequences)", hop_sequence_id, hop_sequence_count);

        // Build and transmit LR-FHSS frame
        match lora_radio.lr_fhss_build_frame(
            &lr_fhss_params,
            hop_sequence_id,
