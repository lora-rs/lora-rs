    ...
    IV: InterfaceVariant + 'static,
{
    ...
    async fn add_register_to_retention_list(&mut self, register: Register) -> Result<(), RadioError> {
        ...
        self.intf.read(&[&[OpCode::ReadRegister.value(), Register::RetentionList.addr1(), Register::RetentionList.addr2(), 0x00u8]], &mut buffer).await?;
    ...
    IV: InterfaceVariant + 'static,
{
    ...
    async fn set_tx_power_and_ramp_time(&mut self, mut power: i8, is_tx_prep: bool) -> Result<(), RadioError> {
        ...
        if self.radio_type == RadioType::SX1261 {
            ...
            self.intf.read(&[&[OpCode::ReadRegister.value(), Register::TxClampCfg.addr1(), Register::TxClampCfg.addr2(), 0x00u8]], &mut tx_clamp_cfg).await?;
    ...
    async fn set_modulation_params(&mut self, mod_params: ModulationParams) -> Result<(), RadioError> {
        ...
        self.intf.read(&[&[OpCode::ReadRegister.value(), Register::TxModulation.addr1(), Register::TxModulation.addr2(), 0x00u8]], &mut tx_mod).await?;
            ...
            */
            ...
            /* ???
                ...
                if receiving_buffer.is_some() && received_len.is_some() {
                    *(received_len.unwrap()) =
                        self.sub_get_payload(receiving_buffer.unwrap()).await?;
                }
...
/*
    ...
    pub(super) async fn sub_get_payload(
        &mut self,
        buffer: &mut [u8],
    ...
    pub(super) async fn sub_set_sleep(
        &mut self,
        sleep_config: SleepParams,
    ) -> Result<(), RadioError> {
        self.iv.disable_rf_switch().await?;
    ...
    pub(super) async fn sub_set_rx(
        ...
        self.brd_write_registers(Register::RxGain, &[0x94u8])
            .await?;
    ...
    pub(super) async fn sub_set_rx_boosted(
        &mut self,
        timeout: u32,
    ) -> Result<(), RadioError> {
        let buffer = [
        ...
        self.brd_write_registers(Register::RxGain, &[0x96u8])
            .await?;
        self.brd_write_command(OpCode::SetRx, &buffer).await?;
        Ok(())
    }
    ...
    pub(super) async fn sub_set_lora_symb_num_timeout(
        &mut self,
        symb_num: u16,
    ) -> Result<(), RadioError> {
        let mut exp = 0u8;
        let mut reg;
        let mut mant =
            ((core::cmp::min(symb_num, SX126X_MAX_LORA_SYMB_NUM_TIMEOUT as u16) as u8) + 1) >> 1;
        while mant > 31 {
            mant = (mant + 3) >> 2;
            exp += 1;
        }
        ...
        self.brd_write_command(OpCode::SetLoRaSymbTimeout, &[reg])
            .await?;

        if symb_num != 0 {
            reg = exp + (mant << 3);
            self.brd_write_registers(Register::SynchTimeout, &[reg])
                .await?;
        }
    ...
    pub(super) async fn sub_get_rx_buffer_status(
        &mut self,
    ) -> Result<(u8, u8), RadioError> {
        if self.packet_params.is_some() {
            let mut status = [0x00u8; 2];
            let mut payload_length_buffer = [0x00u8];

            self.brd_read_command(OpCode::GetRxBufferStatus, &mut status)
                .await?;
            if self.packet_params.unwrap().implicit_header
            {
    ...
    pub(super) async fn sub_get_packet_status(
        &mut self,
    ) -> Result<PacketStatus, RadioError> {
        let mut status = [0x00u8; 3];
        self.brd_read_command(OpCode::GetPacketStatus, &mut status)
            .await?;
        ...
        let freq_error = 0x00;  // self.frequency_error;
